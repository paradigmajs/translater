...
const KEY = 'bc4efd0c24msha5471dbef5b595bp1f4ac5jsnba65a042d713'
...
const goToTranslate=()=>{
    let enterWords = document.getElementById('enter').value
    let source = document.getElementById('source').value
    let target = document.getElementById('target').value
    if(enterWords.length > 1){
        const encodedParams = new URLSearchParams();
        encodedParams.append("q", enterWords);
        ...
        getTranslate(options)
    }else{
        alert('Enter words')
    }
}

const getTranslate = async (options) => {
    try{
        const req = await fetch(API, options)
        const res = await req.json()
        renderTranslate(res.data.translations[0])
    }catch(error){
        console.log(error.message);
    }
   
}
const renderTranslate = ({translatedText})=> output.value = translatedText
